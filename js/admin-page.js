...
function renderLocations(){const q=$('#location-search').value.toLowerCase(),cat=$('#location-category').value;const rows=venues.filter(v=>(cat==='all'||(v.categories||[]).includes(cat)||v.category===cat)&&(!q||v.name.toLowerCase().includes(q)||(v.address||'').toLowerCase().includes(q)||v.id.includes(q))).slice(0,250);$('#locations-body').innerHTML=rows.map(v=>`<tr><td data-label="Lokasi"><strong>${esc(v.name)}</strong><small>${esc(v.address||v.id)}</small></td><td data-label="Kategori">${(v.categories&&v.categories.length?v.categories:[v.category]).map(c=>`<span class="category-pill">${categories[c]?.label||c}</span>`).join()}</td><td data-label="Sumber">${esc(v.source||'OpenStreetMap')}</td><td data-label="Koordinat"><code>${v.lat.toFixed(5)}, ${v.lng.toFixed(5)}</code></td><td data-label="Aksi"><div class="row-actions"><button data-edit="${v.id}">Edit</button><a class="row-map" href="map.html?venue=${encodeURIComponent(v.id)}" target="_blank" rel="noopener">Peta</a><a class="row-sv" href="https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=${(+v.lat).toFixed(7)},${(+v.lng).toFixed(7)}" target="_blank" rel="noopener">Street View</a><button class="delete" data-delete="${v.id}">Hapus</button></div></td></tr>`).join('')||'<tr><td colspan="5" class="empty">Tidak ada lokasi.</td></tr>';$$('[data-edit]').forEach(b=>b.onclick=()=>openLocation(venues.find(v=>v.id===b.dataset.edit)));$$('[data-delete]').forEach(b=>b.onclick=()=>confirmDelete(venues.find(v=>v.id===b.dataset.delete)))}
...
function renderBusinesses(){
 ...
 body.innerHTML=rows.map(b=>{
   ...
   return `<tr><td data-label="Usaha"><strong>${esc(b.name)}</strong><small>${esc(b.address||'')}</small></td><td data-label="Pemilik"><small>${esc(b.owner_name||'—')}</small><small>${esc(b.owner_email||'')}</small></td><td data-label="Layanan">${services}</td><td data-label="Status">${accessible}<br>${statusBadge}</td><td data-label="Aksi"><div class="row-actions"><button class="btn btn-outline" data-biz-verify="${b.id}" data-verified="${b.verified?'1':'0'}">${b.verified?'Batalkan verifikasi':'Verifikasi'}</button><button class="delete" data-biz-delete="${b.id}">Hapus</button></div></td></tr>`;
 }).join('');
...
function renderLeaderboard(){
 ...
 if(rows.length===0){body.innerHTML='';empty.hidden=false;return}
 empty.hidden=true;
 body.innerHTML=rows.map((r,i)=>`<tr><td data-label="#" class="rank">${i+1}</td><td data-label="Kontributor"><strong>${esc(r.name)}</strong></td><td data-label="Jumlah audit">${r.count}</td><td data-label="Poin"><strong>${r.points}</strong></td><td data-label="Aksi"><div class="row-actions"><button class="delete" data-lb-remove="${esc(r.name)}">Hapus</button></div></td></tr>`).join('');
